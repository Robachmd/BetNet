...
Cross-platform mobile client for the BetRent Django API: browse rentals in Ethiopia, save favorites, chat with landlords, manage alerts, and post listings (landlords).

## Prerequisites

- [Flutter SDK](https://docs.flutter.dev/get-started/install) (stable, 3.22+ recommended)
- Android Studio / Xcode for device emulators
- Your BetRent backend running and reachable from the phone or emulator (see networking below)
...
```bash
flutter create . --project-name betrent_app
```
...
flutter run --dart-define=BETRENT_API_BASE=http://192.168.1.50:8000
```

Production:

```bash
flutter build apk --dart-define=BETRENT_API_BASE=https://api.yourdomain.com
```
...
| `services/betrent_api.dart` | Dio client, JWT refresh interceptor, REST calls, `AuthNotifier` |
